...
export default async (client, cluster, worker, ipc, message) => {
  ...
  // check if the message was sent in a channel with autoreact
  // don't run autoreact if we can't add reactions
  if (message.channel.guild && message.channel.permissionsOf(client.user.id).has("addReactions")) {
